...
/* ── Module-level controller (scoped to this module) ── */
...
function _go(step) {
  const v = Math.min(Math.max(step, 0), VOYAGE_TOTAL);
  if (_ctrl) {
 ...
 * }} props
 */
export function renderVoyageTimeline(container, props) {
  const {
    currentStep = 0,
...
function _buildDOM(container) {
  ...
  container.innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;gap:3px;flex-shrink:0">
      <span class="tl-label">航程時間軸</span>
      <span id="vt-mode-badge" style="
    ...
    <div class="tl-btn-group">
      <button class="tl-btn" id="vt-start" title="跳至出發">&#9664;&#9664;</button>
      <button class="tl-btn" id="vt-play"  title="播放 / 暫停">&#9654;</button>
      <button class="tl-btn" id="vt-end"   title="跳至 ETA">&#9654;&#9654;</button>
    </div>

    <div class="tl-track-wrap">
      <div class="tl-events-layer" id="vt-events-layer"></div>
      <div class="tl-win-overlay"  id="vt-win-overlay"></div>
      <div class="tl-anchor-line"  id="vt-anchor-line" style="display:none"></div>
      <input type="range" class="tl-slider" id="vt-slider"
             min="0" max="${VOYAGE_TOTAL}" value="0" step="1" />
      <div class="tl-ticks" id="vt-ticks"></div>
    </div>

    <button class="tl-btn tl-now-btn" id="vt-now" style="display:none">
      &#8617; 回到現在
    </button>

    <span class="tl-time" id="vt-time">${stepLabel(0)}</span>
  `;

  _ctrl = {
    container,
    slider:     container.querySelector("#vt-slider"),
    timeDisp:   container.querySelector("#vt-time"),
    evLayer:    container.querySelector("#vt-events-layer"),
    winOvl:     container.querySelector("#vt-win-overlay"),
    anchorLine: container.querySelector("#vt-anchor-line"),
    ticks:      container.querySelector("#vt-ticks"),
    nowBtn:     container.querySelector("#vt-now"),
    playBtn:    container.querySelector("#vt-play"),
    modeBadge:  container.querySelector("#vt-mode-badge"),
  };

  container.querySelector("#vt-start").addEventListener("click", () => {
    _stopPlay();
    _go(0);
  });
  container.querySelector("#vt-end").addEventListener("click", () => {
    _stopPlay();
    _go(VOYAGE_TOTAL);
  });
  _ctrl.playBtn.addEventListener("click", () => {
    if (_playing) {
      ...
      if (parseInt(_ctrl.slider.value) >= VOYAGE_TOTAL) _go(0);
      _playing = true;
      _ctrl.playBtn.classList.add("active");
      _ctrl.playBtn.innerHTML = "&#9646;&#9646;";
      _timerId = setInterval(() => {
        const cur = parseInt(_ctrl.slider.value);
        if (cur >= VOYAGE_TOTAL) { _stopPlay(); return; }
        _go(cur + 1);
      }, 700);
    }
  });
...
function _update({ currentStep, win, events, mode, focusEventId }) {
  ...
  // Window overlay
  const winL = (start / VOYAGE_TOTAL) * 100;
  const winW = ((end - start) / VOYAGE_TOTAL) * 100;
  _ctrl.winOvl.style.left  = `${winL}%`;
  _ctrl.winOvl.style.width = `${winW}%`;
  _ctrl.winOvl.style.background  = isEvent
    ? "rgba(210,153,34,.10)" : "rgba(88,166,255,.07)";
  _ctrl.winOvl.style.borderColor = isEvent
    ? "rgba(210,153,34,.35)" : "rgba(88,166,255,.2)";

  // Anchor line for EVENT focus
  if (isEvent && focusEventId) {
    const focusHour = parseInt(focusEventId.split("_").pop(), 10);
    const anchorPct = (focusHour / VOYAGE_TOTAL) * 100;
  ...
  _ctrl.ticks.innerHTML = windowTicks.map((h) => {
    const pct = (h / VOYAGE_TOTAL) * 100;
    const cls = h === currentStep ? "tl-tick cur" : "tl-tick";
